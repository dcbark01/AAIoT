...
def StaticDataPath(resource):
    if not os.path.exists(os.path.join('static/example_data', resource)):
        raise Exception(dedent('''
            The file "{}" does not exist in the "static" folder.
        '''.format(resource, resource)))
    if 'DYNO' in os.environ and config.PATH_BASED_ROUTING:
        return '/{}/static/example_data/{}'.format(config.DASH_APP_NAME, resource)
    else:
        return 'static/example_data/{}'.format(resource)